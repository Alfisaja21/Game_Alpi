...
async function showDiscussion(){showScreen('discussion');$('discussionRoomCode').textContent=currentRoomCode;const room=await loadRoom();startTimer(room);if(currentIsHost){$('startVotingBtn').classList.remove('hidden');$('discussionHint').classList.add('hidden')}else{$('startVotingBtn').classList.add('hidden');$('discussionHint').classList.remove('hidden')}}

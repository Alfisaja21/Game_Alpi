...
// ==============================
// WEREWOLF V7 BUTTON HANDLER
// ==============================

(function(){

function initWerewolfButtons(){

    const createButtons = document.querySelectorAll(
        "button, .create-room, .btn-create, #createRoomBtn"
    );

    const joinButtons = document.querySelectorAll(
        "button, .join-room, .btn-join, #joinRoomBtn"
    );

    createButtons.forEach(btn=>{
        if(btn.innerText.toUpperCase().includes("CREATE")){
            btn.onclick = function(e){
                e.preventDefault();
                console.log("CREATE ROOM CLICKED");
                alert("Create Room Werewolf siap dihubungkan ke Supabase 🐺");
            };
        }
    });

    joinButtons.forEach(btn=>{
        if(btn.innerText.toUpperCase().includes("JOIN")){
            btn.onclick = function(e){
                e.preventDefault();
